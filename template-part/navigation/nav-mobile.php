 ...
 *
 * Sliding panel menu with overlay for mobile devices.
 * Supports both accordion and sliding panel styles.
 */

// Get the menu style from the data attribute (set by shortcode)
$menu_style = isset( $args['menu_style'] ) ? $args['menu_style'] : 'accordion';
...
<div class="c-cl-mobile-nav">
	...
	<div id="modal-nav-wrap" class="c-modal-nav-wrap <?php echo esc_attr( $style_class ); ?>" data-menu-style="<?php echo esc_attr( $menu_style ); ?>" tabindex="-1">
		...
		<nav class="c-modal-nav" role="navigation" itemscope itemtype="https://schema.org/SiteNavigationElement">
				...
				<div class="c-modal-nav-logo">
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="nofollow">
						...
						if ( has_custom_logo() ) {
							...
							echo '<img src="' . esc_url( get_template_directory_uri() . '/img/wplogo.png' ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" />';
						}
						?>
					</a>
				</div>
			...
			<?php
			// Output mobile menu using basic wp_nav_menu
			...
			<div class="c-cl-mobile-nav-footer">
				<p>
					...
					<span aria-label="Email">Email:</span> <a href="mailto:info@companyname.com">info@companyname.com</a>
				</p>
